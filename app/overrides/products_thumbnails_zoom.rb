...
Deface::Override.new(:virtual_path => "spree/products/_thumbnails",
                      ...
                      :replace => "code[erb-loud]:contains('link_to')") do
                        "<a href=\"<%= @product.images.first.attachment.url(:product) %>\" rel=\"{gallery: 'product-gallery', smallimage: '<%= i.attachment.url(:large) %>', largeimage: '<%= i.attachment.url(:original) %>'}\" class=\"<%= (i.id == @product.images.first.id) ? 'zoomThumbActive' : '' %> \" > <%= image_tag(i.attachment.url(:mini)) %></a>"
                      end
